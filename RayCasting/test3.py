...
def getShapeWalls(count,radius,center):
    wallList=[]
    point1=[radius+center[0],center[1]]
    for i in range(1,count):
        alpha=(math.pi/180)*(i*360/count)+0.00001
        point2=[math.cos(alpha)*radius+center[0],-math.sin(alpha)*radius+center[1]]
        wallList.append(Wall(point1,point2))
        point1=point2.copy()
    wallList.append(Wall(wallList[-1].point2,wallList[0].point1))
    return wallList
...
def main():
    ...
    run=True
    lightSource=LightSource([300,300],120)
    pressing=False
    ...
    wallList += getShapeWalls(50,50,(1000,300))
