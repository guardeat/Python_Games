...
class Circle():
    ...
    def move(self):
        self.center[0] +=self.speed.ivector/5
        self.center[1] +=self.speed.jvector/5
...
class eventHandler():
    ...
    def collider(circle1,circle2,distance):
        ...
        if distance==0: 
            distance=0.1
        try:    
            i=abs(circle1.center[0]-circle2.center[0])/(circle1.center[0]-circle2.center[0])
            j=abs(circle1.center[1]-circle2.center[1])/(circle1.center[1]-circle2.center[1])
        except:
            i=1
            j=1  
    ...
    def checkBordes(circleList,center,radius):
        ...
        for circle in circleList:
            ...
            if(distance>radius-circle.radius):
                try:
                    i=-abs(circle.center[0]-center[0])/(circle.center[0]-center[0])
                    j=-abs(circle.center[1]-center[1])/(circle.center[1]-center[1])
                except:
                    i=1
                    j=1  
