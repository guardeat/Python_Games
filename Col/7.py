...
class eventHandler():
    ...
    def collider(circle1,circle2,distance):
        ...
        if distance<0.01: 
            return
        try:    
            i=3*(circle1.radius+circle2.radius-distance)*abs(circle1.center[0]-circle2.center[0])/(circle1.center[0]-circle2.center[0])
            j=3*(circle1.radius+circle2.radius-distance)*abs(circle1.center[1]-circle2.center[1])/(circle1.center[1]-circle2.center[1])
        except:
            i=0.01
            j=0.01
    ...
    def mover(circleList):
        for i2 in range(5):
            ...
            eventHandler.checkBordes(circleList,[700,0],600)
            for i in circleList:
                i.move()  
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 4:
                    ...
                    a-=1  
        pygame.draw.circle(screen,(255,0,0),[700,0],600,1)        
