...
class Printer():
    ...
    def printEnds(screen,gameMap):
        for i in gameMap.blockList:
            pygame.draw.rect(screen,(0, 127, 50),i.getRect())  

    def printPlayerPos(screen,gameMap):
        for i in gameMap.blockList:
            pygame.draw.rect(screen,(200, 0, 50),i.getRect())   
...
def main():
    ...
    with open("level_map.txt","w") as levelFile:
        for block in walls.getBlockList(): 
            levelFile.write("w-"+str(block.leftUpCorner[0])+","+str(block.leftUpCorner[1])+","+str(block.height)+","+str(block.width)+"\n")
